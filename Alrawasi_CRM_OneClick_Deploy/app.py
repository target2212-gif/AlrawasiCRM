...
def sync_muqawil(max_pages=8):
 # Daily incremental scan of the newest/listing pages. Existing rows are skipped
 # by membership number, then by normalized company name.
 d=db(); added=updated=seen=errors=0
 try:
  for page in range(1,max_pages+1):
   try:
    r=requests.get(MUQAWIL_LIST,params={'page':page},timeout=25,
                   headers={'User-Agent':'AlrawasiCRM/1.0 (+business CRM sync)'})
    r.raise_for_status()
    cards=_muqawil_cards(r.text)
   except Exception:
    errors+=1; continue
   for card in cards:
    seen+=1
    try:
     info=_muqawil_detail(card['url'])
     name=info['name'] or card['name']
     member=(info['member'] or '').strip() or None
     row=None
     if member:
      row=d.execute('SELECT id FROM companies WHERE muqawil_member_no=?',(member,)).fetchone()
     if not row:
      # Avoid duplicating companies already present in the CRM.
      for x in d.execute('SELECT id,name_ar,name_en FROM companies WHERE name_ar=? OR name_en=?',(name,name)).fetchall():
       if _norm_name(x['name_ar'])==_norm_name(name) or _norm_name(x['name_en'])==_norm_name(name):
        row=x; break
     if row:
      d.execute("""UPDATE companies SET
        muqawil_member_no=COALESCE(muqawil_member_no,?), source_url=?,
        ...
        data_source=COALESCE(data_source,'Muqawil')
        WHERE id=?""",(member,info['url'],info['phone'],info['city'],info['address'],row['id']))
      updated+=1
     else:
      # Match current schema columns dynamically.
      cols={r['name'] for r in d.execute('PRAGMA table_info(companies)').fetchall()}
      vals={'name_ar':name,'name_en':'','activity':'مقاولات','city':info['city'],
            'phone':info['phone'],'whatsapp':info['phone'],'email':'','website':'',
            'cr_number':'','status':'جديد','business_area':'contracting',
            'muqawil_member_no':member,'source_url':info['url'],'address':info['address'],
            'data_source':'Muqawil','imported_at':datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
      keys=[k for k in vals if k in cols]
      d.execute(f"INSERT INTO companies ({','.join(keys)}) VALUES ({','.join('?' for _ in keys)})",
                tuple(vals[k] for k in keys))
      added+=1
     d.commit()
    except Exception:
     d.rollback(); errors+=1
  return {'added':added,'updated':updated,'seen':seen,'errors':errors}
 finally:
  d.close()
...
def sync_muqawil_admin():
 result=sync_muqawil()
 flash(f"مزامنة مقاول: تمت إضافة {result['added']} شركة، تحديث {result['updated']}، أخطاء {result['errors']}.")
