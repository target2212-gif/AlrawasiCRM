...
def reset_import_verified_muqawil():
 ...
 prepared=[]; seen=set(); duplicates=[]; rejected=[]; seen_members=set()
 ...
 try:
  ...
  for no,(x,key,name,url,iid,ctype) in enumerate(prepared,1):
   ...
   member=str(x.get('membershipNumber') or '').strip(); member = member if member and member not in seen_members else ''; seen_members.add(member) if member else None
