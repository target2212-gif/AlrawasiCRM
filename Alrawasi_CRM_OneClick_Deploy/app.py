...
def muqawil_profile_url(row):
 """Open Muqawil's contractor directory filtered to this exact contractor.
 Muqawil currently exposes contractor details as contractor cards in the directory,
 so we use the membership number first, then company name, rather than inventing a profile URL.
 """
 def getv(*keys):
  for k in keys:
   try:
   ...
   if v is not None and str(v).strip():
    return str(v).strip()
  return ''
 member=getv('muqawil_member_no','membership_no','member_no')
 # verification_notes sometimes contains the actual Muqawil membership number.
 if not member:
  notes=getv('verification_notes')
  m=re.search(r'(?:رقم\s*(?:العضوية|عضوية)|MQ[-\s]*)([0-9]{5,})',notes,re.I)
  if m:
   member=m.group(1)
 # Never use internal CRM id/source_id as if it were a Muqawil membership/profile id.
 query=member or getv('name_ar','name_en')
 return 'https://muqawil.org/ar/contractors?q=' + urllib.parse.quote(query)


app.jinja_env.globals['muqawil_profile_url']=muqawil_profile_url
