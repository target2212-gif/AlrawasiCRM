...
def company(cid):
 ...
 its=d.execute("SELECT i.*,COALESCE(u.name,i.user_name_snapshot,'مستخدم محذوف') rep FROM interactions i LEFT JOIN users u ON u.id=i.user_id WHERE company_id=? ORDER BY contacted_at DESC",(cid,)).fetchall(); users=d.execute('SELECT * FROM users WHERE active=1').fetchall() if session.get('role')=='admin' else []; me=d.execute('SELECT * FROM users WHERE id=?',(session['uid'],)).fetchone(); d.close(); msg=wa_text(me,c['name_ar'] or c['name_en']); link=wa_link(c['whatsapp'] or c['phone_mobile'],msg); mail_link=email_link(c['email'],c['name_ar'] or c['name_en'],msg)
...
def google_phone_one(cid):
 ...
 if existing:
  d.close(); flash('لم يتم تغيير الرقم: الشركة لديها رقم مسجل مسبقاً.'); return redirect(safe_return)
