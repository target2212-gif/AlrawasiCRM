...
def companies():
 ...
 sql="""SELECT c.*,u.name rep,(SELECT i.contacted_at FROM interactions i WHERE i.company_id=c.id ORDER BY i.contacted_at DESC LIMIT 1) last_contact,(SELECT i.next_followup_at FROM interactions i WHERE i.company_id=c.id AND i.next_followup_at IS NOT NULL ORDER BY i.contacted_at DESC LIMIT 1) next_followup FROM companies c LEFT JOIN users u ON u.id=c.assigned_user_id"""+w+""" ORDER BY
 CASE WHEN COALESCE(TRIM(c.whatsapp),'')<>'' OR COALESCE(TRIM(c.phone_mobile),'')<>'' THEN 0 ELSE 1 END,
 CASE
 WHEN c.classification_status='غير مصنفة' THEN 0
 WHEN c.classification_status IS NULL OR c.classification_status='' OR c.classification_status='غير محدد' OR c.classification_grade IS NULL OR c.classification_grade='' OR c.classification_grade LIKE '%غير محدد%' THEN 1
 WHEN c.classification_grade='6' OR c.classification_grade LIKE '%سادس%' THEN 2
 WHEN c.classification_grade='5' OR c.classification_grade LIKE '%خامس%' THEN 3
 WHEN c.classification_grade='4' OR c.classification_grade LIKE '%رابع%' THEN 4
 WHEN c.classification_grade='3' OR c.classification_grade LIKE '%ثالث%' THEN 5
 WHEN c.classification_grade='2' OR c.classification_grade LIKE '%ثان%' THEN 6
 WHEN c.classification_grade='1' OR c.classification_grade LIKE '%أول%' THEN 7
 ELSE 8 END, c.id LIMIT ? OFFSET ?"""
...
def company(cid):
 ...
 its=d.execute("SELECT i.*,COALESCE(u.name,i.user_name_snapshot,'مستخدم محذوف') rep FROM interactions i LEFT JOIN users u ON u.id=i.user_id WHERE company_id=? ORDER BY contacted_at DESC",(cid,)).fetchall(); users=d.execute('SELECT * FROM users WHERE active=1').fetchall() if session.get('role')=='admin' else []; me=d.execute('SELECT * FROM users WHERE id=?',(session['uid'],)).fetchone(); display_number=d.execute('SELECT COUNT(*) FROM companies WHERE id<=?',(cid,)).fetchone()[0]; c=dict(c); c['display_number']=display_number; d.close(); msg=wa_text(me,c['name_ar'] or c['name_en']); link=wa_link(c['whatsapp'] or c['phone_mobile'],msg); mail_link=email_link(c['email'],c['name_ar'] or c['name_en'],msg)
...
def reset_import_verified_muqawil():
 ...
 d=db()
 try:
  d.execute('BEGIN IMMEDIATE'); d.execute('DELETE FROM interactions'); d.execute('DELETE FROM companies')
  for no,(x,key,name,url,iid,ctype) in enumerate(prepared,1):
   ...
   (key,name,'مقاولات',str(x.get('city') or '').strip(),str(x.get('address') or '').strip(),'الهيئة السعودية للمقاولين (منصة مقاول)',url,1,
    'new','normal',_muqawil_status(x.get('classificationStatus')),str(x.get('classificationGrade') or '').strip(),str(x.get('membershipNumber') or '').strip(),
...
def muqawil_enrichment_queue():
 ...
 payload={'version':'V7.5.10','type':'muqawil_enrichment_queue','count':len(rows),'companies':[dict(x) for x in rows]}
