...
def _real_muqawil_profile_url(x):
 for k in ('muqawilProfileUrl','muqawil_profile_url','profileUrl','profile_url','contractorUrl','contractor_url','sourceUrl'):
  u=str((x.get(k) if isinstance(x,dict) else '') or '').strip()
  if re.match(r'^https?://(?:www\.)?muqawil\.org/ar/contractors/\d+/\d+(?:[/?#].*)?$',u,re.I):
   return u
 return ''
...
def ensure_schema():
 ...
 try:
  ...
  if 'muqawil_profile_url' not in ccols: d.execute("ALTER TABLE companies ADD COLUMN muqawil_profile_url TEXT")
  if 'muqawil_profile_status' not in ccols: d.execute("ALTER TABLE companies ADD COLUMN muqawil_profile_status TEXT")
  if 'crm_number' not in ccols: d.execute("ALTER TABLE companies ADD COLUMN crm_number INTEGER")
  missing=d.execute("SELECT id FROM companies WHERE crm_number IS NULL ORDER BY id").fetchall()
  next_no=(d.execute("SELECT COALESCE(MAX(crm_number),0) n FROM companies").fetchone()['n'] or 0)+1
  for rr in missing:
   d.execute("UPDATE companies SET crm_number=? WHERE id=?",(next_no,rr['id'])); next_no+=1
  d.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_companies_crm_number ON companies(crm_number)")
...
def import_muqawil_json():
 ...
 try:
  ...
  for x in records:
   ...
   real_profile=_real_muqawil_profile_url(x)
   ...
   vals={'source_id':sid,'name_ar':str(x.get('nameAr') or ''),'name_en':str(x.get('nameEn') or ''),
         ...
         'data_source':'Muqawil','muqawil_member_no':member or None,'muqawil_profile_url':real_profile or None,'muqawil_profile_status':'verified' if real_profile else 'missing','classification_status':cstatus,
...
@app.get('/company/<int:cid>/muqawil')
@login_required
def open_muqawil_company(cid):
 d=db(); c=d.execute('SELECT * FROM companies WHERE id=?',(cid,)).fetchone(); d.close()
 if not c: abort(404)
 u=(c['muqawil_profile_url'] or '').strip()
 if re.match(r'^https?://(?:www\.)?muqawil\.org/ar/contractors/\d+/\d+(?:[/?#].*)?$',u,re.I):
  return redirect(u)
 member=(c['muqawil_member_no'] or '').strip()
 if re.fullmatch(r'MQ-\d+',member,re.I): member=''
 q=member or (c['name_ar'] or '').strip()
 return redirect('https://www.muqawil.org/ar/contractors?q='+urllib.parse.quote(q))
