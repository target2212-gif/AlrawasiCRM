...
def scoped_company(d,cid):
 if session.get('role')=='admin': return d.execute('SELECT c.*,u.name rep FROM companies c LEFT JOIN users u ON u.id=c.assigned_user_id WHERE c.id=?',(cid,)).fetchone()
 return d.execute('SELECT c.*,u.name rep FROM companies c LEFT JOIN users u ON u.id=c.assigned_user_id WHERE c.id=? AND c.assigned_user_id=?',(cid,session['uid'])).fetchone()
...
def companies():
 ...
 if session.get('role')!='admin': where.append('c.assigned_user_id=?'); args.append(session['uid'])
 elif rep:
...
def update_company(cid):
 d=db(); c=scoped_company(d,cid)
 if not c: d.close(); abort(404)
 assigned=(request.form.get('assigned_user_id') or None) if session.get('role')=='admin' else session['uid']
 d.execute('UPDATE companies SET marketing_status=?,assigned_user_id=?,priority=?,marketing_notes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?',(request.form['marketing_status'],assigned,request.form.get('priority','normal'),request.form.get('marketing_notes',''),cid)); d.commit(); d.close(); return redirect(url_for('company',cid=cid))
...
def interaction(cid):
 d=db(); c=scoped_company(d,cid)
 if not c: d.close(); abort(404)
 d.execute('INSERT INTO interactions(company_id,user_id,channel,outcome,notes,next_followup_at) VALUES(?,?,?,?,?,?)',(cid,session['uid'],request.form['channel'],request.form['outcome'],request.form.get('notes',''),request.form.get('next_followup_at') or None)); d.execute('UPDATE companies SET marketing_status=?,assigned_user_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?',(request.form['outcome'],session['uid'],cid)); d.commit(); d.close(); return redirect(url_for('company',cid=cid))
...
@login_required
def export():
 d=db(); rows=d.execute('SELECT * FROM companies ORDER BY id').fetchall() if session.get('role')=='admin' else d.execute('SELECT * FROM companies WHERE assigned_user_id=? ORDER BY id',(session['uid'],)).fetchall(); out=io.StringIO(); w=csv.writer(out); w.writerow(rows[0].keys() if rows else ['id']); [w.writerow(list(r)) for r in rows]; d.close(); return Response('\ufeff'+out.getvalue(),mimetype='text/csv',headers={'Content-Disposition':'attachment; filename=companies.csv'})
