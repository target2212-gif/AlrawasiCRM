...
# DATA-LOCK: on Render the persistent disk is authoritative. Never fall back to BASE/crm.db.
if IS_RENDER and os.path.abspath(DATA_DIR)!='/var/data':
 raise RuntimeError('DATA-LOCK: Render must use DATA_DIR=/var/data')
...
@app.get('/admin/free-safety')
@admin_required
def free_safety():
 return render_template('free_safety.html', is_admin=True)
...
def restore_json_snapshot():
 ...
 if not f or not f.filename.lower().endswith('.json'):
  flash('اختر ملف Snapshot JSON صالح.'); return redirect(url_for('free_safety'))
 try:
  payload=json.load(f)
  if payload.get('format')!='alrawasi_crm_json_snapshot' or not isinstance(payload.get('tables'),dict):
   raise ValueError('صيغة Snapshot غير معتمدة')
  # Structural validation before touching live runtime DB.
  src_tables=payload['tables']
  if 'companies' not in src_tables or 'users' not in src_tables:
   raise ValueError('الملف لا يحتوي الجداول الأساسية')
  backup_path=os.path.join(BASE,'pre_restore_runtime_backup.db')
  src=sqlite3.connect(DB); dst=sqlite3.connect(backup_path)
  try: src.backup(dst)
  ...
  try:
   ...
   d.execute('BEGIN IMMEDIATE')
   existing={r['name'] for r in d.execute("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'")}
   for table,rows in src_tables.items():
    if table not in existing: continue
    safe='"'+table.replace('"','""')+'"'
    d.execute('DELETE FROM '+safe)
    if rows:
     actual=[r['name'] for r in d.execute('PRAGMA table_info('+safe+')')]
     cols=[c for c in rows[0].keys() if c in actual]
     if cols:
      qcols=','.join('"'+c.replace('"','""')+'"' for c in cols)
      qs=','.join('?' for _ in cols)
      d.executemany('INSERT INTO '+safe+' ('+qcols+') VALUES ('+qs+')',
                    [[row.get(c) for c in cols] for row in rows])
   d.commit()
  except Exception:
   d.rollback(); raise
  finally:d.close()
  flash('تمت استعادة Snapshot JSON بنجاح. أُنشئت نسخة SQLite قبل الاستعادة.')
 except Exception as e:
  flash('لم تتم الاستعادة: '+str(e))
 return redirect(url_for('free_safety'))
